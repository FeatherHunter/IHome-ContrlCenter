...
void lwip_dhcp_task(void *pdata)
{
	static int tcp_connect_task_flag = 1;
	...
	/*创建连接和认证任务*/
	if(tcp_connect_task_flag == 1)
	{
		OS_ENTER_CRITICAL();  
		OSTaskCreate(tcp_connect_task,(void*)0,(OS_STK*)&TCP_CONNECT_TASK_STK[TCP_CONNECT_STK_SIZE-1],TCP_CONNECT_TASK_PRIO);//创建任务 
		OS_EXIT_CRITICAL(); 
