 ...
 * @filename exti.c
 * @description 2.4寸/2.8寸/3.5寸/4.3寸/7寸 TFT液晶驱动	  
 ...
 *		2.	void LCD_DisplayOn(void);																		//开显示
 *		3.	void LCD_DisplayOff(void);																	//关显示
 *		4.	void LCD_Clear(u16 Color);	 																//清屏
 *		5.	void LCD_SetCursor(u16 Xpos, u16 Ypos);											//设置光标
 *		6.	void LCD_DrawPoint(u16 x,u16 y);														//画点
 *		7.	void LCD_Fast_DrawPoint(u16 x,u16 y,u16 color);							//快速画点
 *		8.	u16  LCD_ReadPoint(u16 x,u16 y); 														//读点 
 *		9.	void LCD_Draw_Circle(u16 x0,u16 y0,u8 r);						 				//画圆
 *		10.	void LCD_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2);					//画线
 *		11.	void LCD_DrawRectangle(u16 x1, u16 y1, u16 x2, u16 y2);		  //画矩形
 *		12.	void LCD_Fill(u16 sx,u16 sy,u16 ex,u16 ey,u16 color);		   	//填充单色
 *		13.	void LCD_Color_Fill(u16 sx,u16 sy,u16 ex,u16 ey,u16 *color);			//填充指定颜色
 *		14.	void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode);						//显示一个字符
 *		15.	void LCD_ShowNum(u16 x,u16 y,u32 num,u8 len,u8 size);  						//显示一个数字
 *		16.	void LCD_ShowxNum(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode);				//显示 数字
 *		17.	void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p);	//显示一个字符串,12/16字体
 *		18.	void LCD_WriteReg(u16 LCD_Reg, u16 LCD_RegValue);
 *		19.	u16 LCD_ReadReg(u16 LCD_Reg);
 *		20.	void LCD_WriteRAM_Prepare(void);
 *		21.	void LCD_WriteRAM(u16 RGB_Code);
 *		22.	void LCD_SSD_BackLightSet(u8 pwm);					//SSD1963 背光控制
 *		23.	void LCD_Scan_Dir(u8 dir);									//设置屏扫描方向
 *		24.	void LCD_Display_Dir(u8 dir);								//设置屏幕显示方向
 *		25.	void LCD_Set_Window(u16 sx,u16 sy,u16 width,u16 height);	//设置窗口				
 */ 
